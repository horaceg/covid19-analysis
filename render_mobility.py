...
def fetch_google():
    ...
    df = (
        ...
        .apply(
            lambda g: g.reset_index(["country", "category"], drop=True)
            .rolling("7d")
            .mean()
        )
        .reset_index()
...
def make_chart(df, world_topo):
    ...
    map_data = (
        df.set_index(["country", "country_code", "category"])["value"]
        .div(100)
    ...
    foreground = (
        ...
        .transform_lookup(
            ...
            from_=alt.LookupData(
                data=map_data, key="country_code", fields=["country"] + categories
            ),
        )
        .transform_fold(fold=categories, as_=["category", "value"])
    ...
    ts_chart = (
        ...
        .transform_aggregate(value="mean(value)", groupby=["category", "date"])
    )

    chart = (ts_chart | map_chart).properties(
        title="Mobility change by geography, across different categories of places (Variation to baseline)"
    )
    return chart
