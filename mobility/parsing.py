...
def parse_stream(stream):
    ...
    if basex == 0.0:
        ...
        data_raw[:, 1] *= 100 / 60.0
        data_raw = data_raw[data_raw[:, 1] != 0.0]
        if npatches == 1:
            good = True
...
def parse_page(doc, ipage, verbose=False):
    ...
    for line in lines:
        ...
        if (
            all(c not in line for c in categories)
    ...
    goodplots = []
    xrefs = sorted(
        doc.getPageXObjectList(ipage), key=lambda x: int(x[1].replace("X", ""))
    )
    ...
    for m, g in zip(tomatch, goodplots):
        ...
        ts = list(
            map(lambda x: pd.Timestamp(x.split(None, 1)[-1] + ", 2020"), tickdates)
        )
...
def parse_page_total(doc, ipage, verbose=False):
    ...
    for line in lines:
        ...
        if ("needs a significant volume of data") in line:
            continue
        if "Mobility trends " in line or "hubs" in line:
            continue
...
def parse_all(date, us=False):
    pdfglob = (
        glob.glob(f"us_pdfs/{date}/*.pdf") if us else glob.glob(f"pdfs/{date}/*.pdf")
    )
    if us:
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        dorig["category"] = (
            row["category"].replace(" & ", "/").replace(" ", "_").lower()
        )
        dorig["page"] = row["page"]
    ...
    df = pd.DataFrame(data)
    df = (
        df.assign(value=lambda f: f["value"] * (f["change"] / f["changecalc"]))
        .replace("workplaces", "workplace")
        .drop("changecalc", axis=1)
    )

    if not us:
        df = df.rename({"state": "country_geoid", "county": "region"}, axis=1).assign(
            country=lambda f: f["country_geoid"].map(GEO_IDS)
        )
    return df
